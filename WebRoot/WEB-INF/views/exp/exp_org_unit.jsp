...
<form id="queryForm" class="ui-form" method="post"> 
	            <div data-options="region:'north',split:true" style="padding:5px">  
	              <!-- <div class="ftitle">维护信息</div>  -->
		            ...
		            <div class="fitem">
						<label>部门代码:</label>
						<input class="easyui-validatebox"   style="width:155px;" name="unit_code"  >	
						<label>部门名称:</label>
	...
	<table id="service_table_id"  title="角色定义" class="easyui-datagrid" style="width:auto;height:350px"
		...
		<thead>
		    <tr><th data-options="field:'ck',checkbox:true">ck</th>
		          ...
		          <th data-options="field:'parent_unit_id',hidden:true">parent_unit_id</th>
		          <th data-options="field:'chief_position_id',hidden:true">chief_position_id</th>
		          <th data-options="field:'company_desc',width:100,resizable:true,editor:{type:'combobox',options: { required: true} }">公司</th>
				  <th data-options="field:'unit_code',width:100,sortable:true,resizable:true,editor: { type: 'validatebox', options: { required: true,validType:'checkUpper'} }">部门代码</th>
				  <th data-options="field:'description',width:100,sortable:true,resizable:true,editor:'text'">部门名称</th>
				  <th data-options="field:'parent_unit_desc',width:100,sortable:true,resizable:true,editor:'combobox'">上级部门</th>
				  <th data-options="field:'position_desc',width:100,sortable:true,resizable:true,editor:'text'">部门主岗位</th>
...
var g_table_uri='ToDoGo.do?actionId=query@list_unit&dataSorce=xml/exp/exp_org_unit.xml';
var g_table_save_url="xml/exp/exp_org_unit.xml";
     ...
     $(g_table_id)._gridlov({
		  ...
		  callback:function(rowIndex,calldata,edit){
			 ...
			 var lov_data=calldata;
	           $(g_table_id).datagrid('updateRow',{index: rowIndex,row: {parent_unit_id:lov_data['company_id'],company_desc: lov_data['company_desc']}});
		  }
	  }); 
   //上级部门选择列
     $(g_table_id)._gridlov({
		  ...
		  index:index,
		  field:"parent_unit_desc",
		  width:540,
		  height:490,
		  callback:function(rowIndex,calldata,edit){
			 debugger; 
			 var lov_data=calldata;
	           $(g_table_id).datagrid('updateRow',{index: rowIndex,row: {parent_unit_id:lov_data['unit_id'],parent_unit_desc: lov_data['unit_desc']}});
		  }
	  }); 
}
